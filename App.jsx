...
// Versión con Supabase (base de datos en la nube)
// ============================================================
...
// ─── SUPABASE CLIENT (sin librería externa) ───────────────────
const sb = {
  ...
  async upsert(table, body, onConflict) {
    ...
    const r = await fetch(this.url(table, qs), { method:"POST", headers:{...this.headers,"Prefer":"return=representation,resolution=merge-duplicates"}, body:JSON.stringify(body) });
    if (!r.ok) throw new Error(await r.text());
    return r.json();
  },
  async rpc(fn, body={}) {
    const r = await fetch(`${SUPABASE_URL}/rest/v1/rpc/${fn}`, { method:"POST", headers:this.headers, body:JSON.stringify(body) });
    if (!r.ok) throw new Error(await r.text());
    return r.json();
  }
};
...
const I = ({ n, s=20 }) => {
  const m = {
    ...
    warn:     <svg width={s} height={s} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M10.29 3.86L1.82 18a2 2 0 001.71 3h16.94a2 2 0 001.71-3L13.71 3.86a2 2 0 00-3.42 0z"/><line x1="12" y1="9" x2="12" y2="13"/><line x1="12" y1="17" x2="12.01" y2="17"/></svg>,
    db:       <svg width={s} height={s} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><ellipse cx="12" cy="5" rx="9" ry="3"/><path d="M21 12c0 1.66-4 3-9 3s-9-1.34-9-3"/><path d="M3 5v14c0 1.66 4 3 9 3s9-1.34 9-3V5"/></svg>,
  };
  return m[n] || null;
};
...
export default function App() {
  ...
  const [configured, setConfigured] = useState(SUPABASE_URL !== "https://TU-PROYECTO.supabase.co");
  ...
  const addMovimiento = useCallback(async (form) => {
    try {
      ...
      const upsertItem = async (sucId, delta) => {
        ...
        if (existing) {
          ...
          await sb.post("inventario", {
            sucursal_id:sucId, articulo:form.articulo, codigo:form.codigo||"",
            categoria:form.categoria||"Operativos", cantidad:delta,
            marca:form.marca||"S/N", estado:form.estado||"Bueno"
          });
        }
      };
  ...
  }, [sucByName, inventario, loadAll, showToast]);

  const saveArticulo = useCallback(async (sucId, item) => {
    try {
      if (item.id) {
        ...
        if (existing) {
          ...
          showToast("Artículo actualizado (ya existía)");
        } else {
  ...
  const addSucursal = useCallback(async (nombre) => {
  ...
  if (!configured) return <SetupScreen onConfigured={()=>{ setConfigured(true); loadAll(); }} />;
      ...
      <div style={{ padding:"18px 16px 0",background:"linear-gradient(180deg,#0d1626,transparent)" }}>
        <div style={{ display:"flex",alignItems:"center",justifyContent:"space-between" }}>
          ...
          <div style={{ display:"flex",gap:8 }}>
            <button onClick={loadAll} style={{ background:"#1a2235",border:"1.5px solid #252f42",borderRadius:10,padding:8,cursor:"pointer",color:loading?"#f59e0b":"#6b7280",display:"flex",alignItems:"center" }} title="Actualizar">
        ...
        <div style={{ marginTop:8,marginBottom:2,display:"flex",alignItems:"center",gap:6 }}>
          ...
          <span style={{ fontSize:11,color:"#374151" }}>Conectado a Supabase · datos en tiempo real</span>
        </div>
      </div>
...
function SetupScreen({ onConfigured }) {
  return (
    <div style={{ minHeight:"100vh",background:"#080d1a",color:"#e2e8f0",fontFamily:"'DM Sans',sans-serif",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:24 }}>
      <style>{`@import url('https://fonts.googleapis.com/css2?family=DM+Sans:opsz,wght@9..40,400;9..40,600;9..40,700&family=Space+Grotesk:wght@600;700&display=swap');*{box-sizing:border-box;margin:0;padding:0}.card{background:#0f1623;border:1px solid #1a2235;border-radius:16px;padding:20px}`}</style>
      <div style={{ maxWidth:480,width:"100%",display:"flex",flexDirection:"column",gap:20 }}>
        <div style={{ textAlign:"center" }}>
          <div style={{ fontSize:48,marginBottom:12 }}>🗄️</div>
          <h1 style={{ fontFamily:"'Space Grotesk',sans-serif",fontSize:24,fontWeight:700,color:"#f8fafc",marginBottom:8 }}>Conectar Base de Datos</h1>
          <p style={{ fontSize:14,color:"#6b7280",lineHeight:1.6 }}>Esta app usa Supabase para guardar todos tus datos en la nube.</p>
        </div>
        <button onClick={onConfigured} style={{ background:"linear-gradient(135deg,#f59e0b,#d97706)",color:"#0a0e1a",border:"none",borderRadius:12,padding:"14px",fontWeight:700,fontSize:15,cursor:"pointer",fontFamily:"inherit" }}>
          Ya configuré mis credenciales →
        </button>
      </div>
    </div>
  );
  ...
  return (
    <div style={{ display:"flex",flexDirection:"column",gap:14 }}>
      <div style={{ display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:10 }}>
        {[
          { l:"Artículos", v:allItems.length, c:"#f59e0b" },
          { l:"Unidades",  v:totalUnids,      c:"#60a5fa" },
          { l:"Mov. Hoy",  v:movHoy,          c:"#34d399" },
        ].map(s=>(
          <div key={s.l} className="card" style={{ padding:12,textAlign:"center" }}>
            {loading ? <div style={{ display:"flex",justifyContent:"center",padding:"6px 0" }}><Spin size={20}/></div> :
              <div style={{ fontSize:24,fontWeight:800,color:s.c,fontFamily:"'Space Grotesk',sans-serif" }}>{s.v}</div>}
          ...
          return (
            <div key={s.id} className="row" style={{ marginBottom:8 }}>
              ...
              <div style={{ display:"flex",gap:12 }}>
                <span style={{ fontSize:11,color:"#6b7280" }}>{items.length} art.</span>
...
function NuevoTab({ sucursales, onSubmit, showToast }) {
  ...
  const validate = () => {
    ...
    if (form.tipo==="traslado_directo") {
      if (!form.origen) e.origen="Requerido";
      if (!form.destino) e.destino="Requerido";
      if (form.origen&&form.destino&&form.origen===form.destino) e.destino="≠ al origen";
    }
  ...
  if (done) return (
    <div style={{ display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",minHeight:320,gap:14,animation:"fadeIn 0.3s ease" }}>
      ...
      <p style={{ fontSize:13,color:"#6b7280",textAlign:"center" }}>El inventario se actualizó en todos los dispositivos</p>
    </div>
  );
  ...
  return (
    <div style={{ display:"flex",flexDirection:"column",gap:14,maxWidth:560,margin:"0 auto" }}>
          ...
          <div style={{ display:"grid",gridTemplateColumns:"1fr 1fr",gap:9 }}>
            {[{v:"entrada",l:"Entrada al Almacén",ic:"⬇️",d:"Viene de sucursal"},{v:"salida",l:"Salida del Almacén",ic:"⬆️",d:"Va a sucursal"}].map(f=>(
              <button key={f.v} onClick={()=>set("fase",f.v)} style={{ background:form.fase===f.v?"#0c1a2e":"#1a2235",border:`2px solid ${form.fase===f.v?"#60a5fa":"#252f42"}`,borderRadius:12,padding:"12px 8px",cursor:"pointer",color:form.fase===f.v?"#60a5fa":"#94a3b8",display:"flex",flexDirection:"column",alignItems:"center",gap:5,transition:"all 0.2s",fontFamily:"inherit" }}>
                ...
                <span style={{ fontSize:11,fontWeight:600,textAlign:"center" }}>{f.l}</span>
                <span style={{ fontSize:10,color:"#4b5563" }}>{f.d}</span>
              </button>
            ))}
          </div>
        </div>
      ...
      <div className="card" style={{ display:"flex",flexDirection:"column",gap:14 }}>
        ...
        {needsOrigen && (
          <div className="field">
            <label>Origen {errs.origen&&<span style={{ color:"#ef4444" }}>— {errs.origen}</span>}</label>
            <select value={form.origen} onChange={e=>set("origen",e.target.value)} style={{ borderColor:errs.origen?"#ef4444":undefined }}>
              <option value="">Seleccionar...</option>
              {sucursales.filter(s=>s!=="Almacén Principal").map(s=><option key={s} value={s}>{s}</option>)}
            </select>
          </div>
        )}
        {needsDestino && (
          <div className="field">
            <label>Destino {errs.destino&&<span style={{ color:"#ef4444" }}>— {errs.destino}</span>}</label>
            <select value={form.destino} onChange={e=>set("destino",e.target.value)} style={{ borderColor:errs.destino?"#ef4444":undefined }}>
              <option value="">Seleccionar...</option>
              {sucursales.filter(s=>form.tipo==="traslado_tes"?s!=="Almacén Principal":s!==form.origen).map(s=><option key={s} value={s}>{s}</option>)}
            </select>
          </div>
        )}
        <div className="field">
          <label>Artículo {errs.articulo&&<span style={{ color:"#ef4444" }}>— {errs.articulo}</span>}</label>
          <input type="text" placeholder="Nombre del artículo..." value={form.articulo} onChange={e=>set("articulo",e.target.value)} style={{ borderColor:errs.articulo?"#ef4444":undefined }}/>
        </div>
        <div style={{ display:"grid",gridTemplateColumns:"1fr 1fr",gap:12 }}>
          <div className="field">
            <label>Cantidad {errs.cantidad&&<span style={{ color:"#ef4444" }}>— {errs.cantidad}</span>}</label>
            <input type="number" min="1" value={form.cantidad} onChange={e=>set("cantidad",e.target.value)} style={{ borderColor:errs.cantidad?"#ef4444":undefined }}/>
          </div>
...
function InvTab({ inventario, sucursales, sucMap, onSave, onDelete, loading }) {
  ...
  const items = (inventario[sucId]||[]).filter(i=>{
    if (filtCateg&&i.categoria!==filtCateg) return false;
    if (filtEstado&&i.estado!==filtEstado) return false;
    return true;
  ...
  return (
    <div style={{ display:"flex",flexDirection:"column",gap:14 }}>
      ...
      <div style={{ display:"flex",justifyContent:"space-between",alignItems:"center" }}>
        ...
        <button className="btn gold" style={{ padding:"8px 14px",fontSize:13 }} onClick={()=>setModal({})}>
          <I n="plus" s={16}/> Agregar
        </button>
      </div>
      ...
      {loading ? (
        ...
        <div className="card empty">
          ...
          <p style={{ fontWeight:600,marginBottom:6 }}>Sin artículos</p>
          <p style={{ fontSize:12 }}>Toca "+ Agregar" para hacer el inventario físico</p>
        </div>
      ) : items.map(item=>(
        <div key={item.id} className="card" style={{ padding:14 }}>
          ...
          <div className="div"/>
          <div style={{ display:"flex",gap:8 }}>
            <button className="btn ghost" style={{ flex:1,padding:"8px",fontSize:12 }} onClick={()=>setModal({item})}>
              <I n="edit" s={14}/> Editar
            </button>
            <button className="btn danger" style={{ flex:1,padding:"8px",fontSize:12 }} onClick={()=>onDelete(item.id)}>
              <I n="trash" s={14}/> Eliminar
            </button>
          </div>
        </div>
      ))}
      ...
      {modal && (
        <ArticuloModal
          ...
          item={modal.item}
          onSave={item=>{ onSave(sucId,item); setModal(null); }}
...
function ArticuloModal({ sucNombre, item, onSave, onClose }) {
  ...
  const save = ()=>{
    if (!form.articulo.trim()) { setErr({articulo:"Requerido"}); return; }
  ...
  return (
    <Modal title={item?`Editar: ${item.articulo}`:`Agregar a ${sucNombre}`} onClose={onClose}>
      <div style={{ display:"flex",flexDirection:"column",gap:13 }}>
        <div className="field"><label>Artículo {err.articulo&&<span style={{ color:"#ef4444" }}>— {err.articulo}</span>}</label><input type="text" placeholder="Nombre..." value={form.articulo} onChange={e=>set("articulo",e.target.value)} autoFocus style={{ borderColor:err.articulo?"#ef4444":undefined }}/></div>
        <div style={{ display:"grid",gridTemplateColumns:"1fr 1fr",gap:12 }}>
...
function HistTab({ movimientos, loading }) {
  ...
  const filtered = movimientos.filter(m=>{
    ...
    if (filtSuc&&m.origen_nombre!==filtSuc&&m.destino_nombre!==filtSuc) return false;
    return true;
  });
...
function BuscarTab({ inventario, sucMap }) {
  ...
  const grouped = results.reduce((acc,item)=>{
    if (!acc[item.articulo]) acc[item.articulo]=[];
    acc[item.articulo].push(item);
    return acc;
  },{});
      ...
      <div className="card" style={{ padding:14 }}>
        <div style={{ position:"relative" }}>
          ...
          <input type="text" placeholder="Artículo, código, categoría, marca..." value={q} onChange={e=>setQ(e.target.value)} autoFocus
            style={{ background:"#1a2235",border:"1.5px solid #252f42",borderRadius:10,color:"#e2e8f0",padding:"12px 13px 12px 40px",fontSize:14,fontFamily:"inherit",width:"100%",outline:"none",transition:"border-color 0.2s" }}
            onFocus={e=>e.target.style.borderColor="#f59e0b"} onBlur={e=>e.target.style.borderColor="#252f42"}/>
        </div>
      </div>

      {query&&<div style={{ fontSize:12,color:"#6b7280" }}>{results.length} resultado{results.length!==1?"s":""} · {Object.keys(grouped).length} artículo{Object.keys(grouped).length!==1?"s":""}</div>}
          ...
          <div key={art} className="card" style={{ padding:14 }}>
            ...
            {items.map((item,i)=>(
              <div key={i} className="row" style={{ marginBottom:i<items.length-1?7:0 }}>
                <div><div style={{ fontSize:13,fontWeight:500 }}>{item.sucursal}</div><div style={{ fontSize:11,color:"#6b7280",marginTop:2 }}>{item.categoria}{item.marca&&item.marca!=="S/N"?` · ${item.marca}`:""}</div></div>
                <div style={{ display:"flex",alignItems:"center",gap:10 }}>
                  <Badge e={item.estado}/>
                  <div style={{ fontSize:20,fontWeight:800,color:"#f59e0b",fontFamily:"'Space Grotesk',sans-serif",minWidth:28,textAlign:"right" }}>{item.cantidad}</div>
                </div>
              </div>
            ))}
            <div style={{ marginTop:10,paddingTop:10,borderTop:"1px solid #1a2235",fontSize:11,color:"#4b5563",display:"flex",gap:14 }}>
              <span>{items.length} sucursal{items.length!==1?"es":""}</span>
              <span>{[...new Set(items.map(i=>i.categoria))].join(", ")}</span>
            </div>
          </div>
        );
      })}
      ...
      <div className="card">
        ...
        <div style={{ display:"flex",gap:10 }}>
          <input type="text" placeholder="Nombre de la sucursal..." value={nombre} onChange={e=>setNombre(e.target.value)} onKeyDown={e=>e.key==="Enter"&&add()}
            style={{ background:"#1a2235",border:"1.5px solid #252f42",borderRadius:10,color:"#e2e8f0",padding:"11px 13px",fontSize:14,fontFamily:"inherit",flex:1,outline:"none",transition:"border-color 0.2s" }}
            onFocus={e=>e.target.style.borderColor="#f59e0b"} onBlur={e=>e.target.style.borderColor="#252f42"}/>
          <button className="btn gold" style={{ padding:"11px 16px" }} onClick={add} disabled={saving||!nombre.trim()}>
            {saving?<Spin size={18}/>:<I n="plus" s={18}/>}
          </button>
        </div>
      </div>
          ...
          return (
            <div key={s.id} className="row" style={{ marginBottom:8 }}>
              ...
              <div style={{ display:"flex",gap:14,alignItems:"center" }}>
                <span style={{ fontSize:11,color:"#6b7280" }}>{items.length} art.</span>
                <span style={{ fontSize:12,fontWeight:700,color:"#f59e0b" }}>{total} und.</span>
              </div>
            </div>
          );
        })}
