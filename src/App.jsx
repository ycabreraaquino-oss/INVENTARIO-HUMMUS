...
export default function App() {
 ...
 const bom = "";
 const csv = bom + rows.map(r => r.map(c => `"${String(c).replace(/"/g,'""')}"`).join(",") const blob = new Blob([csv], { type:"text/csv;charset=utf-8;" });
 ...
 <span style={{ fontSize:9.5,fontWeight:tab===t.id?700:500,letterSpacing:"0.04em"  </button>
 ))}
 </div>
...
function HomeTab({ inventario, movimientos, sucMap, sucursales, setTab, loading }) {
 ...
 {/* Alertas críticas */}
 {criticos.length>0&&(
 <div style={{ background:"#0f1623",border:"1.5px solid #7f1d1d55",borderRadius:16,pad <div style={{ display:"flex",justifyContent:"space-between",alignItems:"center",mar <p style={{ fontSize:13,fontWeight:700,color:"#fca5a5" }}>⚠ Artículos Críticos</p <span style={{ background:"#7f1d1d44",color:"#f87171",border:"1px solid #7f1d1d", </div>
 ...
 const hora=new Date(m.created_at).toLocaleTimeString("es-DO",{hour:"2-digit",minu
 const dia=new Date(m.created_at).toDateString()===new Date().toDateString()?"Hoy" return (
...
function ArticuloBuscador({ sucursalNombre, inventario, sucByName, value, onChange, onSelect, const [q, setQ] = useState(value||"");
 ...
 {open && sucursalNombre && (sugerencias.length > 0 || enOtras.length > 0 || q.trim()) & <div style={{ position:"absolute",top:"100%",left:0,right:0,background:"#0f1623",bord
 ...
 {confirmar && (
 <div style={{ background:"#1c1708",border:"1.5px solid #f59e0b44",borderRadius:12,pad <p style={{ fontSize:13,color:"#f59e0b",fontWeight:600,marginBottom:10 }}>⚠ "{confi <p style={{ fontSize:12,color:"#94a3b8",marginBottom:12 }}>¿Confirmas usar este art
 <div style={{ display:"flex",gap:8 }}>
...
function NuevoTab({ sucursales, onSubmit, showToast, inventario, sucByName }) {
 ...
 <div style={{ display:"grid",gridTemplateColumns:"1fr 1fr",gap:12 }}>
 <div className="field"><label>Código Activo</label><input type="text" placeholder=" <div className="field"><label>Observaciones</label><input type="text" placeholder="
 </div>
...
function InvTab({ inventario, sucursales, sucMap, onSave, onDelete, loading }) {
 ...
 <div style={{ textAlign:"right",flexShrink:0 }}>
 <div style={{ fontSize:26,fontWeight:800,color:"#f59e0b",fontFamily:"'Space Gro
